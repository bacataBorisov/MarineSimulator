...
- [ ] Apply a known preset such as `Light Weather` so repeated runs start from a stable baseline.
- [ ] Confirm the required sensors are enabled before testing their related sentences from the dashboard.
- [ ] Keep the NMEA console drawer open with timestamps visible so observed reader behavior can be matched to emitted lines.
...
- [ ] Verify disabling the anemometer removes both wind controls and wind sentence emission paths from the practical test flow.
...
- [ ] Verify when both gyro and magnetic heading are enabled, derived calculations follow the gyro source rather than magnetic fallback.
...
- [ ] Verify disabling GPS removes GPS positioning control from the dashboard and suppresses GPS-derived sentences in the reader.
