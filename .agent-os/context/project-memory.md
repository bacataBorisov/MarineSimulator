...
- `NMEASimulator` is the central observable engine/state object; views consume it via SwiftUI environment or `@Bindable`. Do not extract stored properties onto a second observable. Idle and transmit share `performSimulationCycle`; idle still applies full state immediately (console writes now), transmit still publishes value-only at 10 Hz. Do not set `transmitRuntime` on the idle path.
