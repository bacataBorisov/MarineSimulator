...
extension NMEASimulator {
    ...
    private func buildGPSSentence(talkerID: String) -> String {
        ...
        let latString = FormatKit.formatLatitude(gpsData.latitude)
        let lonString = FormatKit.formatLongitude(gpsData.longitude)
        
        let cogString = String(format: "%.1f", gpsData.courseOverGround)
        let sogString = String(format: "%.1f", gpsData.speedOverGround)
